...
public class BulletScript : MonoBehaviour {
	...
	*/
    ...
    // 슬라이더의 값을 바로 가져와서 적용하기 위해
    // 필요한 슬라이드 컨트롤 스크립트 변수
    public SlideControl sd;

    private void Start()
    {
		gunScript = FindObjectOfType<GunScript>();
        // 인자로 true 값을 주면 비활성화 상태인 요소도 가져올 수 있다.
        sd = FindObjectOfType<SlideControl>(true);
    ...
    void Update () {
        ...
        errorRotation = Quaternion.Euler(gunScript.UpDownError + sd.xSlider.value, gunScript.LeftRightError + sd.ySlider.value, 0f);
        errorDirection = errorRotation * transform.forward;

        // 보통 메서드는 결과 값을 return으로 하나만 돌려줄 수 있다.
        // 하지만, 타격 여부와 별개로 어디에, 누구에게 어디에서 같은 많은 정보가 필요하기 때문에
