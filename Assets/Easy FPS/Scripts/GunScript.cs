...
public class GunScript : MonoBehaviour
{
    ...
    // Random gun error value
    // 상하좌우 오차를 생성

    // x의 로테이션 값과 관련이 있다.
    public float UpDownError = 0.0f;
    // y의 로테이션 값과 관련있다.
    public float LeftRightError = 0.0f;
    // => Bullet 스크립트에서 참조해야하기 때문에 public 선언
    ...
    {
        // 오차 각도 조절 
        // Bullet 스크립트에서 넣으면, 총알을 생성할 때마다 오차가 변해서
        // 오차를 수정할 수 없다. 따라서, 처음 한번 생성하는 Gun의 스크립트에 작성
        // 그리고 해당 오차를 Bullet 스크립트에서 가져가는 느낌으로 구현
        UpDownError = Random.Range(-5f, 5f);
        LeftRightError = Random.Range(-5f, 5f);
        ...
        // 현재 시간을 시드 값으로 하여 매번 다른 랜덤 패턴이 나오도록 유도
	...
	*/
    void Update()
    {
        ...
        // 5초에 한번씩 불발 이벤트와 관련된 랜덤 함수 호출
        if (misBulletTimer > 5.0f)
        {
            // 0 ~ 9 사이 정수
            misRand = Random.Range(0, 10);

            // 그 중 3개 (0, 1, 2) 즉, 30% 확률로 불발 이벤트 발생
            if (misRand < 5)
            {
                // missLoced 불리언 변수를 true로 바꾸는 부분
                PlayerManagement.Instance.missLocked = true;
            }
	...
	*/
    void MeeleAttack()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Q) && !meeleAttack)
        {
            ...
            PlayerManagement.Instance.missLocked = false;
        }
    }

    // 리스트를 이용해 불발탄 관리 (불발 상황을 여러번 발생시키기 위해)
    public List<GameObject> misBullets = new List<GameObject>();

    // 불발 횟수
    int misCnt = 0;

    // 불발탄 모션을 구현하는 메서드
    public void EjectMisbullet()
    {
        if (misCnt < 5)
        {
            misBullets[misCnt].SetActive(true);
            // 부모 자식 관계 해제
            // 게임 엔진에서 자식 오브젝트는 부모의 상대좌표를 따른다.
            // 따라서 총알이 공중에 있다 캐릭터가 회전하면 그 방향을 따라 움직일 수 있다.
            // 그리고, 부모로 묶여있으면 연산을 계속해서 해야하기 때문에 불필요한 연산과 물리 버그가 생길 수 있다.
            misBullets[misCnt].transform.SetParent(null);

            // 물리 연산 활성화
            Rigidbody rb = misBullets[misCnt].GetComponent<Rigidbody>();
            rb.isKinematic = false;
            rb.useGravity = true;

            // 옆으로 튕겨나가는 힘 전달
            Vector3 ejectDir = transform.right + transform.up;
            // Force 모드에서 Force는 서서히 힘을 가하는 방식
            // Impulse는 한번에 힘을 가하는 방식
            rb.AddForce(ejectDir * 2f, ForceMode.Impulse);
            // AddForce가 물체를 직선으로 밀어낸다면
            // AddTorque는 물체를 회전시키는 힘을 주는 메서드
            // AddTorque(newVector3(x, y, z), ForceMode.Impulse);
    ...
    private void ShootMethod()
    {
        if (waitTillNextFire <= 0 && !reloading && pmS.maxSpeed < 5)
        {
            ...
            if (bulletsInTheGun > 0)
            {
                ...
                Quaternion errorRotation = Quaternion.Euler(UpDownError, LeftRightError, 0f);
                Vector3 finalDirection = errorRotation * bulletSpawnPlace.transform.forward;
                if (bullet)
                    Instantiate(bullet, bulletSpawnPlace.transform.position, Quaternion.LookRotation(finalDirection));
                else
